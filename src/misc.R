...
#rangeAfter
x = byunby(frac, frac[ , "patient_num"], FUN = rangeAfter, 60, "start_date", "<=", T, F,T, fstart = tmp, fend= ntmp, clip = T, strict = T)
y = byunby(frac, frac[ , "patient_num"], FUN = rangeAfter, 60, "age_at_visit_days", "<=", T, F,T, fstart = tmp, fend= ntmp, clip = T, strict = T)
